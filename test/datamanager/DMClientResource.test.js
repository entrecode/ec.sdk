...
describe('DMClient ListResource', () => {
  let listJson;
  let list;
  before(() => {
    return new Promise((resolve, reject) => {
      fs.readFile(`${__dirname}/../mocks/dm-client-list.json`, 'utf-8', (err, res) => {
        if (err) {
          return reject(err);
        }
...
describe('DMClient Resource', () => {
  ...
  const getter = ['clientID', 'callbackURL', 'hexColor', 'disableStrategies', 'tokenMethod', 'grantTypes', 'federatedOIDC'];
  getter.forEach((name) => {
  ...
  const setter = ['callbackURL', 'hexColor', 'disableStrategies', 'tokenMethod', 'grantTypes', 'federatedOIDC'];
  setter.forEach((name) => {
